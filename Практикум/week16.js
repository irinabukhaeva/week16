...
function makeOne() {
  //Ваш код
  paragraph.textContent = `Количество найденных форм: ${document.forms.length}`;
}
...
function makeTwo() {
  //Ваш код
  paragraphTwo.textContent = `Значение атрибута name первой формы: ${document.forms[0].getAttribute(
    "name"
  )}`;
}
...
function makeThree() {
  //Ваш код
  paragraphThree.textContent = `Значение атрибута name последней формы: ${document.forms[3].getAttribute(
    "name"
  )}`;
}
...
function makeSeven() {
  ...
  for (i = 0; i < formElements.length; i++) {
    elementsList[i] = `- ${formElements[i].getAttribute("name")}`;
  }
...
function makeTen() {
  ...
  console.log(document.forms[3].elements);
}
...
function makeEleven() {
  ...
  let select = form.elements.firstSelect;
  const optionsValues = [];
  for (i = 0; i < select.length; i++) {
    optionsValues[i] = select[i].value;
  }
...
function makeTwelve() {
  ...
  let checkbox1 = form.elements[2];
  let checkbox2 = form.elements[3];
  let checkbox3 = form.elements[4];
  paragraphTwelve.textContent = `${checkbox1.getAttribute(
    "id"
  )}, ${checkbox2.getAttribute("id")}, ${checkbox3.getAttribute("id")}`;
}
...
function checkButton(e) {
  ...
  if (radio.checked) paragraphThirteen.textContent = "Кнопка выбрана";
...
function checkOption() {
  ...
  else paragraphFourteen.textContent = "Выбран третий вариант";
}
...
formOne.addEventListener("submit", function (event) {
  //Отмена отправки
  let firstName = formOne.elements.firstName;
  let firstEmail = formOne.elements.firstEmail;
  let errorMessage = document.getElementById("errorMessage");
  if (firstName.value == "") {
    errorMessage.textContent = "Поле Имя не заполнено";
    event.preventDefault();
  } else if (firstEmail.value == "") {
    errorMessage.textContent = "Поле email не заполнено";
    event.preventDefault();
  }
  //Ваш код
});
...
emailInput.oninput = function () {
  ...
  if (emailRegexp.test(emailInput.value)) {
    ...
    errorMessage.textContent = "Email введен некорректно ";
  }
};
...
radio.addEventListener("change", () => button.removeAttribute("disabled"));
...
document.querySelector(".b-26").onclick = function (event) {
  event.preventDefault();
  document.getElementById("result26").textContent =
    document.forms.formThree.elements.thirdName.getAttribute("placeholder");
  //Ваш код
};
